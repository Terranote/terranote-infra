...
if [ "$EUID" -eq 0 ]; then
    ...
    if ! sudo -n true 2>/dev/null; then
        ...
        echo "Si el usuario terranote no tiene permisos sudo configurados, el script fallará"
        echo ""
    fi
    SUDO_CMD="sudo"
...
# Verificar si ya existe (necesitamos sudo para leer el archivo)
if $SUDO_CMD grep -q "$HOSTNAME" "$CONFIG_FILE"; then
    echo "✓ El hostname $HOSTNAME ya está configurado"
    echo ""
    echo "Configuración actual:"
    $SUDO_CMD grep -A 2 "$HOSTNAME" "$CONFIG_FILE"
    echo ""
